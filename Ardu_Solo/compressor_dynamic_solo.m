...
addpath('../', '-end');
currentFolder = pwd;
% model_file = strcat(currentFolder, '/model_px4.mat');
...
for n=1:N-1
    dt = t(n+1) - t(n);
    [dx(:,n),y(:,n)] = quadrotor_m_solo(t(n), x(:,n), u(:,n), a,b,c,d, m, I_x, I_y, I_z, K_T, K_Q);
    x(:,n+1) = x(:,n) + dx(:,n) * dt; 
...
for n=1:N-1
    dt = t(n+1) - t(n);
    [dx(:,n),y2(:,n)] = quadrotor_m_solo(t(n), x(:,n), u(:,n), a,b,c,d, m, I_x, I_y, I_z, K_T, K_Q);
    x(:,n+1) = x(:,n) + dx(:,n) * dt; 
    x(6,n+1) = mod(x(6,n+1), 2*pi); % wrap yaw to [0,2pi)
