...
reference_steering = train_data(find(abs(train_data(:, 14))>0.5,1), 14) - 0.5;
...
raw_states(:, [2,3]) = raw_states(:, [2,3]) - raw_states(1, [2,3]);
...
for n=1:N-1
    dt = t(n+1) - t(n);
    [dx(:,n),y(:,n), terms] = rover_m_erle(t(n), x(:,n), u(:,n), m,a,b,Cx,Cy,CA);
...
%for last y
[dx(:,N),y(:,N)] = rover_m_erle(t(N), x(:,N), u(:,N), m,a,b,Cx,Cy,CA);
