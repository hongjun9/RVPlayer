...
raw_motors = train_data(:,14:17);   %(((0.5595*1000)+1000)-1100)/900=0.51
...
m = 0.8;
I_x = 0.005; %16365151e-9; %16365151e-9;      % Inertia (kg*m^2)
I_y = 0.005; %8354114e-9;    %8354114e-9;
I_z = 0.009; %24008439e-9;  %24008439e-9;


% alpha = 9.6;
% throttle_hover = 0.5; %0.5595;%0.51; %0.7;    % (%)  (1559-1100)/900=0.51
...
for n=1:N-1
    dt = t(n+1) - t(n);
    [dx(:,n),y(:,n)] = quadrotor_m_px4(t(n), x(:,n), u(:,n), a,b,c,d, m, I_x, I_y, I_z, K_T, K_Q);
    x(:,n+1) = x(:,n) + dx(:,n) * dt; 
...
%     x([4:6, 10:12],n+1) = states([4:6, 10:12],n+1); % synchronize for attitude
...
Filename       = 'quadrotor_m_px4';                % File describing the model structure.
